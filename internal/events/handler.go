...
func (h *Handler) List(c *fiber.Ctx) error {
	...
	if err != nil {
		return respondError(c, fiber.StatusInternalServerError, "INTERNAL_ERROR", "Erreur lors de la récupération des événements.")
	}
...
func (h *Handler) Get(c *fiber.Ctx) error {
	...
	if err != nil {
		if errors.Is(err, ErrNotFound) {
			return respondError(c, fiber.StatusNotFound, "NOT_FOUND", "Événement introuvable.")
		}
		return respondError(c, fiber.StatusInternalServerError, "INTERNAL_ERROR", "Erreur lors de la récupération de l'événement.")
	}
...
func mapError(c *fiber.Ctx, err error) error {
	...
	if errors.Is(err, ErrInvalidStatus) {
		return respondError(c, fiber.StatusBadRequest, "VALIDATION_ERROR", "Statut invalide. Valeurs acceptées : planning, setup, running, strike, completed.")
	}
